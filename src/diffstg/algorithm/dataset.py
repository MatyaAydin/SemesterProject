...
def search_recent_data(train, label_start_idx, T_p, T_h):
    ...
    """
    if label_start_idx + T_p > len(train): return None # prediction goes beyond the data range
    start_idx, end_idx = label_start_idx - T_h, label_start_idx - T_p + T_p # should be - T_h ?
...
def search_multihop_neighbor(adj, hops=5):
    """Return a distance matrix, the distance is bounded by `hops` and set to -1 otherwise."""
    node_cnt = adj.shape[0]
    hop_arr = np.zeros((adj.shape[0], adj.shape[0])) # Same shape as adj

    for h_idx in range(node_cnt):  # refer node idx(n)

        # Initialize idx(n) row:
        ...
        for hop_idx in range(hops):  # how many spatial steps
            tmp_step_node = []  # neighbor nodes in the previous k step (all nodes found)
            tmp_step_node_kth = []  # neighbor nodes in the kth step (new unvisited nodes)
    ...
    return hop_arr[:, :, np.newaxis] # np.newaxis explanation: https://stackoverflow.com/questions/29241056/how-do-i-use-np-newaxis

class CleanDataset():
    def __init__(self, config):
        """
        config: dictionary specific to the dataset (path, number of nodes, etc.)
        """
    ...
    def read_data(self):
        if 'PEMS' in self.data_name:
            ...
            data = np.expand_dims(np.load(self.feature_file)[:, :, 0], -1)
            data = np.nan_to_num(data, nan=0) # Fillna
        elif 'Metro' in self.data_name:
            data = np.expand_dims(np.load(self.feature_file)[:, :, 0], -1)
            data = np.nan_to_num(data, nan=0)

        elif 'EWZ' in self.data_name:
            data = np.expand_dims(np.load(self.feature_file)[:, :, 0], -1)
    ...
    def interaction_range_mask(self, hops=2, t_size=3):
        """Define the neighborhood within the range of spatial hops for the convolution"""
        hop_arr = self.spatial_distance
        hop_arr[hop_arr != -1] = 1 # within the range
        hop_arr[hop_arr == -1] = 0 # out of range
        return np.concatenate([hop_arr.squeeze()] * t_size, axis=-1)  # V,tV



class TrafficDataset(Dataset): # Inherited from torch.utils.data.Dataset
    def __init__(self, clean_data, data_range, config):
        """
        clean_data: CleanDataset object
        """
    ...
    def __getitem__(self, index): # overwrites the method from Dataset
    ...
    def __len__(self): # overwrites the method from Dataset
